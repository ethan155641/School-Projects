...
public partial class _Default : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        {
            // Bonus: Show cookie info and explain parsing
            // The cookie.Value is a string like "eMail=xxx&DOB=yyy"
            // We split on '&' to get each key-value pair
            string[] rawInput = cookie.Value.Split('&');
            // We split the first pair on '=' to get the email value
            string[] eMail = rawInput[0].Split('=');
            // Display the email address
            Response.Write("If " + eMail[1] + " is not your email address, please click on Reset<br />");
            // Block comment: We use split to separate key-value pairs and extract the email value for display.
        }
        if (IsPostBack)
        {
